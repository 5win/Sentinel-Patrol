# Sentinel Patrol
 ...
 ROS2 기반 순찰 로봇 프로젝트입니다.  

 단순히 "움직이는 로봇"을 만드는 데서 멈추지 않고, **위험 감지**, **안전 정지**, **상태 전이**, **회피 기동**, **비전 인식**까지 연결되는 구조를 직접 설계하고 검증하고 있습니다.
...
- Nav2 기반 순찰 주행이 가능한 ROS2 로봇 시스템 구현
- LiDAR 기반 전방 위험 감지(`Scan Logger`)와 독립적인 안전 정지 계층(`Safety Gate`) 구축
- 단순 조건문이 아니라 상태머신(State Machine) 중심으로 순찰 흐름 설계(`Patrol Manager`)
- 장애물 회피 후 다시 순찰로 복귀하는 Avoidance 루프 구성
- YOLO 기반 Person detection 파이프라인을 추가해 상위 행동 확장 기반 마련(`Patrol Vision`)
...
- Nav2 주행과 안전 제어를 분리해 독립적인 safety layer를 만들었습니다.
- 불안정한 동작을 파라미터 조정으로 덮지 않고 상태 전이 문제로 재해석했습니다.
- 회피 동작을 임시 명령이 아니라 action 기반 흐름으로 구조화했습니다.
- 비전 결과를 단발성 테스트가 아니라 ROS 메시지 파이프라인으로 연결했습니다.
...
### 3. 정지 후 재출발이 반복되는 oscillation 문제
...
- 사람 인식 결과를 patrol manager와 연결해 행동 전환까지 확장
- 동적 장애물 환경에서 avoidance 성능 검증
- waypoint 순찰 정책 고도화
- edge case 데이터 기반 vision threshold 및 모델 튜닝
